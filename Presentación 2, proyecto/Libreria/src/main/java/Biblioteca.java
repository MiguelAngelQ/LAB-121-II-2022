...
public class Biblioteca {
    ...
    public void ordenar_empleados_id(){
        for(int i = 0; i < nro_empleados - 1; i++){
            for(int j = i + 1; j <  nro_empleados; j++){
                if(emp[j].getId_empleado() > emp[i].getId_empleado()){
                    Empleado x = emp[j];
                    emp[j] = emp[i];
                    emp[i] = x;
                }
